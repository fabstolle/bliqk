...
export default function SettingsPage() {
  ...
  const Sec = ({title,children}:{title:string;children:React.ReactNode}) => (
    <div style={{background:t.card,borderRadius:16,padding:"22px 24px",border:`1px solid ${t.border}`,marginBottom:16}}>
      <div style={{fontSize:10,fontWeight:700,color:t.textSecondary,textTransform:"uppercase" as const,letterSpacing:1.2,marginBottom:18}}>{title}</div>
      {children}
    </div>
  );
  ...
  return (
    <div style={{ padding:"32px 36px", maxWidth:700, color:t.textPrimary }}>
      <h2 style={{ margin:"0 0 4px", fontSize:22, fontWeight:700, fontFamily:"'Instrument Serif',serif" }}>{s.settingsTitle} ⚙️</h2>
      <p style={{ margin:"0 0 24px", fontSize:fs.sm, color:t.textBody }}>{s.settingsSub}</p>

      {/* ── Themes ── */}
      <Sec title={s.themeSection}>
          ...
          {(Object.keys(THEMES) as ThemeId[]).map(id => {
            ...
            return (
              <div key={id} onClick={()=>setTheme(id)}
                ...
                {/* Mini preview */}
                <div style={{ height:60, background:th.bg, display:"flex" }}>
                  <div style={{ width:26, background:th.surface, borderRight:`1px solid ${th.border}`, padding:"5px 4px", display:"flex", flexDirection:"column", gap:3 }}>
      ...
      <Sec title={s.fontSection}>
        ...
        <div style={{ background:t.bg, borderRadius:10, padding:"12px 16px", border:`1px solid ${t.border}` }}>
          ...
          <div style={{ fontSize:fs.xs, color:t.textSecondary, marginTop:3 }}>
            {language==="en"?"Labels and timestamps at XS.":"Etiquetas en tamaño XS."}
          </div>
        </div>
      </Sec>

      {/* ── Contrast ── */}
      <Sec title={s.contrastSection}>
      ...
      <div style={{ marginTop:14, textAlign:"center", fontSize:10, color:t.textSecondary }}>{s.autoSaved}</div>
    </div>
  );
}
