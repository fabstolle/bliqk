...
export default function AppLayout({ children }: { children: React.ReactNode }) {
  ...
  const NAV = [
    ...
    { href:"/app/capture",      icon:"✍️", label:s.smartCapture },
    { href:"/app/integrations", icon:"🔌", label:s.integrations },
    { href:"/app/usage",        icon:"📊", label:s.usageCosts },
    ...
    <div style={{ display:"flex", height:"100vh", background:t.bg, color:t.textPrimary, fontFamily:"'DM Sans',sans-serif", overflow:"hidden" }}>
      ...
      <div style={{ width:224, background:t.surface, borderRight:`1px solid ${t.border}`, display:"flex", flexDirection:"column", flexShrink:0 }}>
        ...
        <div style={{ padding:"18px 16px 14px", borderBottom:`1px solid ${t.border}` }}>
          <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:4 }}>
            ...
            <div style={{ display:"flex", gap:5 }}>
              <button onClick={() => setLanguage(language==="en"?"es":"en")}
                style={{ fontSize:13, background:"transparent", border:`1px solid ${t.border}`, borderRadius:6, padding:"2px 6px", cursor:"pointer", color:t.textSecondary }}>
                {language==="en"?"🇲🇽":"🇺🇸"}
              </button>
          ...
          <div style={{ fontSize:9, color:t.textSecondary, letterSpacing:0.8 }}>
            {language==="en"?"SEE WHAT NEEDS ACTION":"VE LO QUE NECESITA ACCIÓN"}
          </div>
        </div>
        ...
        {critical > 0 && (
          <div style={{ margin:"8px 8px 0", background:`${t.danger}12`, border:`1px solid ${t.danger}25`, borderRadius:8, padding:"7px 11px" }}>
            ...
            <div style={{ fontSize:10, color:t.textSecondary, marginTop:1 }}>Requires immediate action</div>
          </div>
        )}
