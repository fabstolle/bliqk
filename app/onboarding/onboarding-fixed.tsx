...
export default function OnboardingPage() {
  ...
  const runCustom = async () => {
    ...
    try {
      ...
      setCustomResult({ score:30, color:"#6B7394", recommendedAction:"Review when available.", reason:"Could not classify.", alerts:[] });
    }
  ...
  return (
    <div style={{ minHeight:"100vh", background:t.bg, color:t.textPrimary, fontFamily:"'DM Sans',sans-serif", position:"relative", overflow:"hidden" }}>
      ...
      <div style={{ position:"relative", zIndex:1, maxWidth:800, margin:"0 auto", padding:"0 24px 80px" }}>
        ...
        {/* ── NAV BAR ── */}
        <div style={{ padding:"24px 0 40px", display:"flex", alignItems:"center", justifyContent:"space-between" }}>
          ...
          {/* Controls — always visible in center-right */}
          <div style={{ display:"flex", alignItems:"center", gap:6 }}>

            {/* Theme dots */}
            <div style={{ display:"flex", gap:4, padding:"5px 8px", background:t.card, borderRadius:10, border:`1px solid ${t.border}` }}>
              {(Object.keys(THEMES) as ThemeId[]).map(id => (
                <button key={id} onClick={() => setTheme(id)}
                  title={language==="en" ? THEMES[id].name : THEMES[id].nameEs}
                  style={{ width:20, height:20, borderRadius:5, border:`2px solid ${theme===id?THEMES[id].accent:"transparent"}`, background:THEMES[id].bg, cursor:"pointer", position:"relative", transition:"transform 0.15s", transform:theme===id?"scale(1.2)":"scale(1)", flexShrink:0 }}>
            ...
            <button onClick={() => setLanguage(language==="en"?"es":"en")}
              style={{ padding:"6px 12px", borderRadius:9, border:`1px solid ${t.border}`, background:t.card, color:t.accent, fontSize:12, fontWeight:700, cursor:"pointer", fontFamily:"inherit", whiteSpace:"nowrap" as const }}>
              {language==="en"?"🇲🇽 ES":"🇺🇸 EN"}
            </button>

            {/* Dark/light */}
            <button onClick={() => setTheme(isDark?"dawn":"midnight")}
              style={{ width:34, height:34, borderRadius:9, border:`1px solid ${t.border}`, background:t.card, cursor:"pointer", fontSize:15, display:"flex", alignItems:"center", justifyContent:"center", flexShrink:0 }}>
              {isDark?"☀️":"🌙"}
            </button>
            ...
            <div style={{ display:"flex", alignItems:"center", gap:4, marginLeft:8 }}>
              {STEPS.map((_,i) => (
                <div key={i} style={{ width:i===step?18:6, height:6, borderRadius:99, background:i<step?t.success:i===step?t.accent:t.border, transition:"all 0.4s cubic-bezier(.4,0,.2,1)", boxShadow:i===step?`0 0 8px ${t.accent}80`:"none" }}/>
              ))}
        ...
        {step===2 && (
          <div style={{ animation:"fadeUp 0.5s ease" }}>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12 }}>
              <button onClick={() => setStep(3)} style={{ padding:"14px", borderRadius:14, border:"none", background:connectPick?`linear-gradient(135deg,${t.accent},#00D4FF)`:t.surface, color:connectPick?"#fff":t.textTertiary, fontSize:15, fontWeight:700, cursor:connectPick?"pointer":"default", fontFamily:"inherit", boxShadow:connectPick?`0 6px 28px ${t.accent}38`:"none" }}>
                {connectPick ? s.connectBtn : (language==="en"?"Select a provider first":"Selecciona un proveedor primero")}
              </button>
              <button onClick={() => setStep(3)} style={{ padding:"14px", borderRadius:14, border:`1px solid ${t.border}`, background:"transparent", color:t.textSecondary, fontSize:14, fontWeight:500, cursor:"pointer", fontFamily:"inherit" }}>
                {s.skipBtn}
              </button>
            </div>
          </div>
        ...
        {step===3 && (
          <div style={{ textAlign:"center", animation:"fadeUp 0.5s ease" }}>
            ...
            <button onClick={() => router.push("/login")} style={{ width:"100%", padding:"16px", border:"none", borderRadius:14, background:`linear-gradient(135deg,${t.accent},#00D4FF)`, color:"#fff", fontSize:16, fontWeight:700, cursor:"pointer", fontFamily:"inherit", boxShadow:`0 8px 36px ${t.accent}42`, marginBottom:12 }}>
              {s.openInbox}
            </button>
